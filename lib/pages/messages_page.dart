...
class _MessagesState extends State<Messages>
  ...
  void initState() {
    ...
    if (kDebugMode) {
      ...
      print(
          'MESSAGES PAGE => INDEX: ${widget.pageIndex} => BOOL: $isInitialized');
    }
  ...
  Future refresh() async {
    ...
    for (int i = 0; i < 8; i++) {
      if (postsList[i].isEmpty) {
        postsList[i] = await Network.GET(
            Network.API_SEARCH_LIST,
            Network.paramsSearch(
                page: page, per_page: 5, topic: topics.values.toList()[i]));
      } else {
        postsList[i].addAll(await Network.GET(
            Network.API_SEARCH_LIST,
            Network.paramsSearch(
                page: page, per_page: 5, topic: topics.values.toList()[i])));
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                ...
                IconButton(
                  icon: const Image(
                    image: AssetImage('assets/icons/other_materials/img.png'),
                    width: 23,
                  ),
                  onPressed: () {},
                ),
      ...
      body: TabBarView(
        ...
        children: <Widget>[
          isInitialized
              ? Scrollbar(child: Updates())
              : const Center(
                  child: CircularProgressIndicator(color: Colors.black)),
          Scrollbar(child: buildMessages()), // Changed the function name here
        ],
      ),
    );
  }
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  const Text('Share ideas with your friends',
                      style:
          ...
          Align(
            ...
            child: Padding(
              ...
              child: MaterialButton(
                ...
                child:
                    const Icon(Icons.edit_sharp, color: Colors.white, size: 40),
                onPressed: () {},
              ),
            ),
          )
...
Widget pin1(int number, BuildContext context, Map<String, String> topics,
    List<List<Pinterest>> postsList) {
  return Container(
    height: 335,
    width: MediaQuery.of(context).size.width,
    ...
    child: Column(
      ...
      children: [
        Padding(
          ...
          child: Text(topics.keys.toList()[number],
...
Widget pin2(int number, BuildContext context, Map<String, String> topics,
    List<List<Pinterest>> postsList) {
  return Container(
    height: 185,
    width: MediaQuery.of(context).size.width,
...
Widget pin3(BuildContext context, Map<String, String> topics,
    List<List<Pinterest>> postsList) {
  return Container(
    ...
    child: Column(
      ...
      children: [
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 10.0),
          child: Text(topics.keys.toList()[3],
              style: const TextStyle(
                  color: Colors.black,
                  fontWeight: FontWeight.w500,
                  fontSize: 15)),
        ...
        Expanded(
          child: SizedBox(
            ...
            child: GridView(
              ...
              children: [
                LoadImage(postsList[3][1], topics.values.toList()[3], context,
                    [true, false, false, false], 0),
                LoadImage(postsList[4][0], topics.values.toList()[4], context,
                    [false, false, true, false], 0),
                LoadImage(postsList[5][3], topics.values.toList()[5], context,
...
Widget LoadImage(Pinterest post, String topic, BuildContext context,
    List<bool> borders, double _height) {
  Color getColorFromHex(String hexColor) {
    hexColor = hexColor.replaceAll("#", "");
    if (hexColor.length == 6) {
      hexColor = "ff" + hexColor;
