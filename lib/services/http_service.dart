...
class Network {
  ...
  static List<Pinterest> parsePost(String responseBody) {
    ...
    List<Pinterest> posts =
        list.map((model) => Pinterest.fromJson(model)).toList();
    return posts;
  }
  ...
  static List<Pinterest> parseSearchPost(String responseBody) {
    ...
    List<Pinterest> photos =
        List<Pinterest>.from(json["results"].map((x) => Pinterest.fromJson(x)));
    return photos;
  }
  ...
  static Future<List<Pinterest>> GET(
    ...
    if (response.statusCode == 200) {
      return api == API_SEARCH_LIST
          ? compute(parseSearchPost, response.body)
          : compute(parsePost, response.body);
    } else if (response.statusCode == 404) {
      throw Exception('Not Found');
    } else {
      throw Exception('Can\'t get post');
  ...
  static Map<String, dynamic> paramsGET(
    ...
    Map<String, String> params = {};
    params.addAll(
        {"page": page.toString(), 'per_page': '20', 'order_by': sortBy});
    return params;
  }
