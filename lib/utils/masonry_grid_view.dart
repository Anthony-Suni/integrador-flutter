...
import 'package:firebase_auth_ui/models/pinterest_models.dart';
...
class _PostState extends State<Post> {
  ...
  void _bottomSheet(BuildContext context, String url) {
    showModalBottomSheet(
      ...
      context: context,
      builder: (context) =>
          widget.pageIndex == 0 ? _bottomSheetView(url) : const SizedBox(),
    );
  }
  ...
  Future refresh() async {
    ...
    List list = widget.pageIndex == 0 && widget.tabIndex == 0
    ...
    if (mounted) {
      setState(() {
        page += 1;
        postsList = List<Pinterest>.from(list);
  ...
  Widget build(BuildContext context) {
    return SmartRefresher(
      ...
      onLoading: () async {
        ...
        postsList.addAll(list as Iterable<Pinterest>);
        if (mounted) {
          setState(() {
